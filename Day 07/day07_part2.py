...
def smallest_sufficent_dir(dir_sizes: dict[str, int]) -> int:
    """Retruns the size of the smalles

    Args:
        dir_sizes (dict[str, int]): Dictionary that contains the sizes of all directories

    Returns:
        int: smales possible size of a directory to be deleted
    """
    total_space = 70_000_000
    requierd_space = 30_000_000
    used_space = dir_sizes['/']
    return min(
        size for size in dir_sizes.values()
        if total_space - used_space + size >= requierd_space
    )
...
def main():
    ...
    print(smallest_sufficent_dir(dir_sizes))
